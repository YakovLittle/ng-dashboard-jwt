        ...
        var State = (function () {
            function State($stateProvider, $locationProvider, $urlRouterProvider, $httpProvider) {
                ...
                $stateProvider
                    ...
                    .state('cres', {
                    url: '/statistics/cres',
                    templateUrl: '/htt/stat_cres.html',
                    controller: App.Controllers.CresCtrl
                })
                    .state('stat', {
                    url: '/stat',
        ...
        var BackendAPI = (function () {
            function BackendAPI($http, $state, sessionService, $rootScope) {
                ...
                this.API = {
                    ...
                    'cres-mon': {
                        'url': '/dataload/statistics/cres',
                        'file': '/api/cres_monitoring.json',
                        'args': '&windowSize=50&pageNumber=1',
                        'wrapper': 'cresSubmissionResults'
                    }
                };
            }
...
(function (App) {
    ...
    (function (Locale) {
        ...
        Locale.i18n = {
            ...
            monitoringCRES: "CRES Monitoring"
        };
...
(function (App) {
    ...
    (function (Controllers) {
        'use strict';
        var CresCtrl = (function () {
            function CresCtrl($scope, $filter, BackendAPI, $modal) {
                this.$scope = $scope;
                this.$filter = $filter;
                this.BackendAPI = BackendAPI;
                this.$modal = $modal;
                this.msg = '';
                $scope.vm = this;
                this.msg = this.$filter('i18n')('monitoringCRES');
                this.uiGrid = new App.Models.GridCRESMon();
            }
            CresCtrl.prototype.updateCRESMon = function () {
                var _this = this;
                this.uiGrid = new App.Models.GridCRESMon();
                this.BackendAPI.doGET('cres-mon')
                    .then(function (data) {
                    _this.uiGrid.loadData(data);
                    _this.lastUpd = new Date();
                })
                    .catch(function (reason) {
                    _this.BackendAPI.go2Page('logout', 'cres');
                });
                ;
            };
            CresCtrl.prototype.openServices = function (row) {
                console.log(row);
            };
            CresCtrl.$inject = ['$scope', '$filter', 'BackendAPI', '$modal'];
            return CresCtrl;
        })();
        Controllers.CresCtrl = CresCtrl;
    })(Controllers = App.Controllers || (App.Controllers = {}));
})(App || (App = {}));
var App;
(function (App) {
...
(function (App) {
    ...
    (function (Models) {
        'use strict';
        var GridCRESMon = (function () {
            function GridCRESMon() {
                this.enableGridMenu = true;
                this.gridMenuShowHideColumns = false;
                this.exporterMenuCsv = true;
                this.exporterMenuPdf = false;
                ...
                this.enableFiltering = true;
                this.showGridFooter = true;
                this.exporterCsvFilename = 'cres_monitoring.csv';
                this.appScopeProvider = {
                    onClick: function (row) {
                        angular.element('#bCRESMon').controller().openServices(row.entity);
                    }
                };
                this.data = [];
                this.columnDefs = [
                    { field: 'pid', enableColumnMenu: false, displayName: 'PID',
                        cellTemplate: '<div class="ui-grid-cell-contents"><a href ng-click="grid.appScope.onClick(row)">{{ row.entity.pid }}</a></div>'
                    },
                    { field: 'startTime', enableColumnMenu: false, displayName: 'Start time', sort: { direction: 'desc', priority: 0 } },
                    { field: 'endTime', displayName: 'End Time', enableColumnMenu: false },
                    { field: 'notificationFim', enableColumnMenu: false, displayName: 'Notification to FIM',
                        cellTooltip: function (row, col) {
                            return row.entity.fimStart + ' => ' + row.entity.fimEnd;
                        }
                    },
                    { field: 'importerFim', enableColumnMenu: false, displayName: 'Importer',
                        cellTooltip: function (row, col) {
                            return row.entity.importerStart + ' => ' + row.entity.importerEnd;
                        }
                    },
                    { field: 'converterFim', enableColumnMenu: false, displayName: 'Converter' },
                    { field: 'enrichmentFim', enableColumnMenu: false, displayName: 'Enrichment' },
                    { field: 'exporterFim', enableColumnMenu: false, displayName: 'Exporter' },
                    { field: 'handshakeFim', enableColumnMenu: false, displayName: 'Handshake' }
                ];
            }
            GridCRESMon.prototype.loadData = function (data) {
                for (var i in data) {
                    this.data.push({
                        pid: data[i].cresPidResult.pid,
                        startTime: (data[i].startTime) || '',
                        endTime: (data[i].endTime) || '',
                        notificationFim: (data[i].notificationFim.processingStatus) || '',
                        fimStart: (data[i].notificationFim.startTime) || '',
                        fimEnd: (data[i].notificationFim.endTime) || '',
                        importerFim: (data[i].importerFim.processingStatus) || '',
                        importerStart: (data[i].importerFim.startTime) || '',
                        importerEnd: (data[i].importerFim.endTime) || '',
                        converterFim: ('converterFim' in data[i] ? data[i].converterFim.processingStatus : ''),
                        enrichmentFim: ('enrichmentFim' in data[i] ? data[i].enrichmentFim.processingStatus : ''),
                        exporterFim: (data[i].exporterFim.processingStatus) || '',
                        handshakeFim: (data[i].handshakeFim.processingStatus) || '',
                        cob: data[i].cresPidResult.cob,
                        runId: data[i].cresPidResult.runId,
                        submId: data[i].cresPidResult.submId,
                        feedId: data[i].cresPidResult.feedId,
                        feedInstcId: data[i].cresPidResult.feedInstcId,
                        cdccId: data[i].cresPidResult.cdccId,
                        cdccInstcId: data[i].cresPidResult.cdccInstcId,
                        bucketInstcId: data[i].cresPidResult.bucketInstcId
                    });
                }
            };
            return GridCRESMon;
        })();
        Models.GridCRESMon = GridCRESMon;
    })(Models = App.Models || (App.Models = {}));
})(App || (App = {}));
var App;
(function (App) {
